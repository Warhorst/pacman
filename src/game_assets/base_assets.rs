...
pub fn start_base_assets_load(
    ...
    commands.insert_resource(BaseAssetsToLoad::from_keys_and_handles([
        ...
        load(GHOST_HOUSE_ENTRANCE, &asset_server),
        load(DOT, &asset_server),
        load(ENERGIZER, &asset_server),
        load(CHERRY, &asset_server),
        load(STRAWBERRY, &asset_server),
        load(PEACH, &asset_server),
        load(APPLE, &asset_server),
        load(GRAPES, &asset_server),
        load(GALAXIAN, &asset_server),
        load(BELL, &asset_server),
        load(KEY, &asset_server),
        load(PACMAN_LIFE, &asset_server),
